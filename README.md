# Gaëtan Martinez — Portfolio

> *Bienvenue sur mon portfolio, je suis Gaëtan, data analyst avec 2 ans d'expérience. Je transforme des données brutes en décisions métier.*

---

## 👋 Qui je suis

Après 10 ans dans la banque et le support client, j'ai fait le choix de me reconvertir dans la data.
Aujourd'hui chez **Elements Apps** à Toulouse,
je travaille sur de vraies problématiques de production : pipelines de données, automatisation, visualisation.

Ce qui me différencie : je comprends les enjeux métier avant de toucher à la donnée.

---

## 💼 Actuellement

**Junior Data Analyst @ Elements Apps** *(sept. 2024 – présent)*

- Pipelines de données sur stack **AWS** (Lambda, S3, RDS PostgreSQL)
- Automatisation de workflows avec **N8N**
- Dashboards de performance sur **Redash**
- Optimisation de modèles **dbt** sur PostgreSQL (index composites, réécriture de CTEs)

---

## 🛠️ Stack technique

| Catégorie | Outils |
|---|---|

| Langages | Python · SQL · Javascript |
| Data Engineering | dbt · PostgreSQL · AWS (Lambda, S3, RDS) |
| Automatisation | N8N · KNIME |
| Visualisation | Power BI · Redash · Plotly · Seaborn |
| Analyse | pandas · numpy · scikit-learn |
| Autres | Git · Excel |

---

## 📂 Projets formation (OpenClassrooms — Data Analyst RNCP 6)

| # | Projet | Sujet | Stack | Compétences clés |
|---|---|---|---|---|

| P2 | **Analyse de ventes e-commerce** | Synthèse graphique des ventes | Excel | Datavisualisation · Narration de données |

| P3 | **Requêtes SQL — Assurance** | Analyse du marché des assurances habitation | SQL | Modélisation BDD · Requêtes métier |

| P4 | **Étude de santé publique — FAO** | Analyse des données sur la faim dans le monde | Python · pandas | Exploration · Analyse exploratoire |

| P5 | **Base de données immobilière** | Conception et requêtes SQL pour une agence immo | SQL | RGPD · Modélisation · Requêtes avancées |

| P6 | **Bottleneck — Gestion des données boutique** | EDA sur données de stock et ventes (vins & spiritueux) | Python · pandas · Plotly | Nettoyage · Fusion · Analyses uni/multivariées |

| P7 | **Dashboard Power BI — Gestion de projet** | Suivi d'avancement de projets pour une entreprise | Power BI | Reporting · DAX · Visualisation interactive |

| P8 | **Profils sociodémographiques — dbt** | Analyse de profils avec workflow dbt | dbt · SQL | Data Engineering · Nettoyage · Agrégation |

| P9 | **Tendances du streaming musical** | EDA + tests statistiques + prévision avec Prophet | Python · Prophet · scipy | Séries temporelles · Tests statistiques |

| P10 | **Eau potable dans le monde** | Dashboards pour une ONG sur l'accès à l'eau | Power BI | Storytelling · Accessibilité · Dashboard interactif |

| P11 | **Étude de marché — Clustering** | Segmentation et ACP pour recommandations stratégiques | Python · scikit-learn | Clustering · ACP · Machine Learning non supervisé |

| P12 | **Détection de faux billets** | Modèle de prédiction par ML supervisé et non supervisé | Python · scikit-learn | Régression logistique · KMeans · ML supervisé |

---

## 🎓 Formation

- **Data Analyst RNCP 6** — OpenClassrooms *(2024 – 2026)*
- **Développeur Web RNCP 6** — Le Wagon *(2023)*
- **DEUG Économie & Gestion** — Toulouse School of Economics *(2010 – 2012)*

---

## 🌍 Langues

Français (natif) · Anglais (professionnel) · Espagnol (professionnel)

---

## 📬 Me contacter

[![LinkedIn](https://img.shields.io/badge/LinkedIn-Gaëtan_Martinez-blue?style=flat&logo=linkedin)](https://www.linkedin.com/in/gaetanmartinez)
[![Email](https://img.shields.io/badge/Email-Contact-red?style=flat&logo=gmail)](mailto:gaetan.martinez81100@gmail.com)

---

*Disponible pour échanger sur des projets data, des opportunités ou simplement partager des idées.*
